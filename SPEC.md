...
**Publishing Target:** GitHub Pages (`garyohosu.github.io/writer2`)  
...
### 5.2 承認待ち
- `pending/` は `manual_review` 用の承認待ちメタデータ置き場とする
- `pending/` は正本ではない
- 各エントリは `stories/` 正本を参照する `*.pending.json` とし、コピーやシンボリックリンクは使わない
...
project-root/
  ...
  pending/
    2026-03-09-midnight-cat.pending.json
  logs/
    2026-03-09/
      run.log
      review.json
...
### 6.1 補足
- `artifacts/context/` には当日実行用の文脈 bundle を保存する
- 例: 直近作品要約、`used_themes` のスナップショット、文体メモ、レビュー入力キャッシュ
- `artifacts/context/` の内容は再生成可能なキャッシュであり、正本ではない
...
### 8.6 カウンタ
- `quality_retry_count` はレビュー不合格による本文再生成回数のみを数える
- Codex CLI のタイムアウト・ネットワーク障害・クラッシュなどの実行失敗は `quality_retry_count` に含めない
...
- しきい値は `0.40` とし、`>= 0.40` の場合は Review Agent が差し戻し候補とする
...
### 14.1 評価軸
- `review_score` の採点対象: 独自性、読みやすさ、一貫性、冒頭の引き、結末の余韻
- ゲート判定: 禁止事項抵触、AdSense適合性

### 14.2 合格条件
- `review_score = round(独自性×0.25 + 読みやすさ×0.20 + 一貫性×0.25 + 冒頭の引き×0.15 + 結末の余韻×0.15)`
- 独自性: 80点以上
- 読みやすさ: 75点以上
- 一貫性: 85点以上
- 総合: 80点以上
...
### 14.3 再生成
- 最大3回まで再生成する
- レビュー不合格時のみ `quality_retry_count` を加算する
- 3回失敗時は `stage=review, result=failed`
- Windows 通知 + ログ保存を行う

### 14.4 実行失敗時の再試行
- Codex CLI のタイムアウト、ネットワーク障害、非ゼロ終了、応答破損は品質リジェクトと別カテゴリで扱う
- 同一 stage / 同一サブステップ内で 2回まで即時再試行し、待機時間は `60秒`、`300秒` とする
- それでも失敗した場合は `stage` を維持したまま `result=failed` とし、標準出力・標準エラー・例外情報をログへ保存する
- 次回 cron 実行時は同一 stage から再開し、`quality_retry_count` は増やさない
...
### 19.2 実行失敗後の再開
- `plot` / `story` / `review` の各 stage で実行失敗した場合は、`result=failed` で停止し次回は同一 stage から再開する
- 既存 artifact が壊れていない限り、直前に成功した stage を巻き戻さない
...
### 21.1 必須
- AdSense ローダースクリプトを `site/_includes/head.html` に1回だけ埋め込む
- 広告枠は共通 include 化し、トップ・一覧・詳細の各テンプレートから呼び出す
- `ads.txt` をルートへ配置する
- `privacy-policy.md` を設置する
- `contact.md` を設置する
- AI生成作品である旨は全ページ共通フッターに常設し、作品詳細ではメタ情報直下にも明記する
...
5. `baseurl=/writer2` 前提でリンク・canonical が生成される
...
13. `pending/` はコピーではなく `*.pending.json` のメタデータで運用される
14. Codex CLI 実行失敗は品質再生成と分離して再試行できる
